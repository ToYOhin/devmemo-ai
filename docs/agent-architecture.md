...
#### R6-I5C refusal boundary

R6-I5C implements the R6-I5B reviewed pure, fixed intent
...
Python domain, Go BFF validator, and Web parser/locale are updated together.
Positive and near-miss policy tests, endpoint projection tests, Go strict-decoder
tests, Web parser/render tests, and the 64-case baseline cover the synchronized
contract. Unknown trace states, step names, Provider values, or refusal text
still fail closed. Rollback removes the policy and new enum/step from all three
consumers together; no data migration or persistent cleanup is needed.
