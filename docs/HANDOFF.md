...
本项目当前结构、产品边界与公开发布状态的权威接管快照为 [`docs/handoffs/2026-07-28-project-structure-release-handoff.md`](handoffs/2026-07-28-project-structure-release-handoff.md)。它确认 Memos Go 是权限与原始 Memo 事实源，AI Service 只保存派生 SQLite，Web 的 AI 产品入口在 `MemoView`；并记录默认 deterministic + memory、低 CPU Compose、Phase 10/11 的证据边界和过期 graphify 图谱限制。

仓库与稳定 `v0.1.0` 已公开，private vulnerability reporting 已启用。独立 GHCR Container package 也已公开；未登录 Docker 客户端已复核 `stable` OCI index 包含 `linux/amd64`、`linux/arm64` 与 `linux/arm/v7`。发行收尾证据见 [`docs/handoffs/2026-07-28-ghcr-public-closeout-handoff.md`](handoffs/2026-07-28-ghcr-public-closeout-handoff.md)；后续没有默认实现任务，必须由用户明确选择新的受控切片。新窗口直接使用 [`docs/prompts/NEXT_STAGE_PROMPT.md`](prompts/NEXT_STAGE_PROMPT.md)。

## 公开稳定发布与 GHCR 匿名拉取闭环（2026-07-28）

- `main` 已包含 PR #1 的验证代码，稳定 [v0.1.0](https://github.com/ToYOhin/devmemo-ai/releases/tag/v0.1.0) 已发布；六个二进制资产、校验清单和多架构稳定镜像在 GitHub Release workflow 通过。
- 稳定 Windows ZIP 的 SHA-256、解压和 `devmemo-ai.exe --help` 已低负载验证。仓库现为 public，private vulnerability reporting 的 GitHub API 返回 `enabled=true`。
- 独立 `devmemo-ai` Container package 已在 GitHub Packages 设置中设为 public。未登录 Docker 客户端的 `docker buildx imagetools inspect ghcr.io/toyohin/devmemo-ai:stable` 成功返回 OCI index `sha256:86a099ceb6e8752aceec8517574840ec0df97730945d0843b5b0305df782dd06`，其中包含 `linux/amd64`、`linux/arm64` 与 `linux/arm/v7`。
- 发行可用性已闭环；详情见 [`docs/handoffs/2026-07-28-ghcr-public-closeout-handoff.md`](handoffs/2026-07-28-ghcr-public-closeout-handoff.md)。没有默认实现任务；不要改动产品 defaults、公共 API 或 `AI_PUBLIC_CHUNK_RETRIEVAL=false`，除非用户明确选择新切片。
...
- 正式公开发布仍被外部治理条件阻塞：仓库 private，且私密漏洞报告渠道未确认。`RELEASE_PLEASE_TOKEN` 缺失只会安全跳过自动 proposal，不阻塞手工稳定发布。当前登录 token 缺少 private Packages `read:packages`，本机直接 registry inspect 的 `403` 不代表 workflow 发布失败。详见 [`docs/handoffs/2026-07-28-rc-release-validation-handoff.md`](handoffs/2026-07-28-rc-release-validation-handoff.md) 与 [`docs/release-preflight.md`](release-preflight.md)。不自动改可见性、配置 token、推稳定 tag 或发布正式版。
...
- 维护者仍需完成 GitHub 外部动作：决定公开可见性后启用/验证私密漏洞报告渠道。当前 secrets 列表为 0，仓库仍为 private；真实 CI、RC Release asset 与 GHCR 已在本次交接顶部复核，不得把这些证据延伸为公开稳定发布已获批准。专用 `RELEASE_PLEASE_TOKEN` 仅在需要自动 proposal 时再配置。
...
## Phase 10 DevMemory local webhook evidence (2026-07-20)

- The authenticated local Memos user now has one webhook to the Docker-network AI Service. Memos runs with `--allow-private-webhooks` because the Compose service name resolves to a private address; this is limited to the local development topology and does not disclose a signing secret to the browser.
- An ordinary authenticated UI update to the existing non-sensitive test Bug Report produced a real webhook delivery. AI Service normalized Memos' `memos/<uid>` resource name to the detail-route UID, persisted one pending Insight, and the already-authorized single review changed it to `accepted` at version `2`.
- The earlier zero-Insight report read the host-default SQLite path, not the Compose-mounted database. For a running Compose service use `docker compose exec -T ai-service python -m scripts.devmemory_lifecycle_report`; it is still read-only and aggregate-only, and after this run reported eight processed events. No Memo ID, raw content, raw payload, or secret is logged.
