...
Before any route-A or route-B work, default local resource use is constrained: Memos `0.75` CPU, AI Service `0.25` CPU, one Go processor, and explicit Qdrant/Ollama profiles. This changes only local scheduling/default startup; deterministic + memory, public chat, and all Phase 10 gates remain unchanged.
