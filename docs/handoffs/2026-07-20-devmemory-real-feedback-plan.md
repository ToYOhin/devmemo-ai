...
3. A real participant is available and agrees to the state-changing review/delete/revoke and clipboard steps. Do not use an API call to bypass Memos authentication.
...
3. The participant reviews the visible Insight. Record one accepted or rejected action and its visible version/status. If independent candidates make both actions meaningful, capture one accept and one reject; otherwise state why only one is present.
