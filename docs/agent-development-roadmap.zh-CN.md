...
与 tool port 执行，checkpoint 稳定 attempt key、retry state、active-time accounting 与安全 outcome。restart
会在任何 attempt 前重新检查 current authority；tool 返回后若 authority 或 cancellation 失效，则丢弃未提交
输出。它仍未接 route、background worker、UI、配置/默认值、Memo write tool、external Provider、真实数据或
多实例。
