...
默认下一切片：Phase 10 route B 已完成，先读取 NEXT_STAGE_PROMPT 选择新的明确范围。不要重复既有测试 Memo 的 Capture/Review/Copy/feedback 路径；没有真实受信任网关、Memos 可见范围映射、回滚与兼容证据时，不开启 public-chunk，不扩展浏览器客户端签名，也不新增通用 RAG/Agent/MCP/图数据库。
