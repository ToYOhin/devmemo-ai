...
当前默认阶段是 Phase 5d：可选 chunk 索引生命周期。Phase 4/4b/4c/4d/4e/4f/4g/5a/5b/5c 的 RAG、HMAC、outbox、显式有限重试、ops 安全、保留预览、告警轮询、显式清理批准、审计、离线检索评估、纯函数 chunking 边界和 OfflineChunkIndex 对照已经完成，必须保持：
- 默认 deterministic + memory，低 CPU、无网络依赖。
- FastEmbed/Qdrant 只进入 adapters，不进入 domain/service。
- 当前生产路径一个完整 Memo 对应一个向量；`MemoChunk`/`OfflineChunkIndex` 仅用于离线试验，不接入 Webhook、Qdrant 或 FastEmbed；RAG `/api/ai/chat` 已完成，但不加入 rerank 或前端聊天 UI。
