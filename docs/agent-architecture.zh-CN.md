...
R6-I5C 实现 R6-I5B 已评审、只处理 verified question 的 pure fixed intent classifier，只允许拒绝明确的
...
Python domain、Go BFF validator 与 Web parser/locale 已同步更新，并覆盖 positive/near-miss policy、endpoint projection、
Go strict decoder、Web parser/render 和 64-case baseline。未知 trace state/step name、Provider 或 refusal text 继续 fail
closed。rollback 必须同步删除三端 policy/enum/step，无 data migration 或持久清理。
...
#### R6-I4B 已评审 runtime matrix

R6-I4B 不修改 runtime code；任何 caller 获得授权前，先固定以下 ownership 与实施顺序：
...
#### R6-I4E retrieval timing 边界

R6-I4E 实现 R6-I4D 已评审的 retrieval-only 边界，为 `EvidenceAnswerAgent` 增加两个 keyword-only dependency：既有
recorder protocol 与 `Callable[[], float]` monotonic clock。现有 constructor 默认两者为 `None`；只有 enabled
internal answer handler 注入 lifespan-owned recorder 与 `time.monotonic`。adapter 绝不持有 clock。
...
已实现 wrapper 只在 signed request 与 tool call 有效后 start，在不搬动分支逻辑的前提下包住现有 branch，由该 branch
...
R6-I4E 实现仅扩展 `agent_observability_runtime.py`，为 `EvidenceAnswerAgent._run` 增加 optional keyword-only dependency
与共享 try/finally boundary，并由 internal answer handler 传入当前 lifespan recorder 和 `time.monotonic`。pure/fake 测试
覆盖 clock validation、memory/durable success、no-context、invalid/unavailable failure、Provider exclusion、dependency
...
#### R6-I4G Provider timing 边界

R6-I4G 实现 R6-I4F 已评审的 `EvidenceAnswerAgent._answer` configured-Provider owner；deterministic
fallback 没有 Provider call，因此不得产生 Provider sample。该实现复用 Agent 的 optional recorder 与 monotonic
...
实现复用 retrieval timing 的整组丢弃规则：dependency 缺失、clock 抛错、bool/非数字/非有限 clock value、负 elapsed 或
...
已实现首批只有一个 helper，以及 configured `generate` call/result-envelope check 外的一层 inner try/finally。post-Provider
